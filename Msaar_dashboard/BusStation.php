...
$BusStation = mysqli_query($con, "SELECT * FROM busstation");
$busStation=array();
if (isset($_POST['submit'])) { // assuming the search term is submitted via a form
    $search_term = $_POST['search_term'];
// Build the SQL query
$sql = "SELECT * FROM busStation WHERE ";
...
// Fetch the data from SQL server 
    ...
    while ($row = mysqli_fetch_array($BusStation)){
        array_push($busStation, $row);
        }}
...
?>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        <a href="addEditStation.php" class="btn btn-primary"><i class="plus"></i>إضافة محطة</a>
    ...
    <div class="table-responsive">
        ...
        <tbody>
        <?php if(!empty($busStation)){ $count = 0; foreach($busStation as $row){ $count++; ?>
            <tr>
                <td><?php echo $row['Id_Station']; ?></td>
                <td><?php echo $row['Address']; ?></td>
                <td><a href="TrackDetails.php?id=<?php echo $row['Id_Track']; ?>"><?php echo $row['Id_Track']; ?></a></td>
                <td><?php $count2 = 0; foreach($region as $row2){ $count2++; if($row['Id_Region']==$row2['Id_Region']){ echo $row2['Name'];}} ?></td>
                <td><?php $count2 = 0; foreach($State as $row3){ $count2++; if($row['Id_State']==$row3['Id_State']){ echo $row3['Name'];}} ?></td>
               
                <td>
                <a href="addEditStation.php?id=<?php echo $row['Id_Station']; ?>" class="btn icon icon-left btn-primary"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>تعديل</a>
             <?php if($row['Id_State']==2){ ?>
                    <a href="UserActionStation.php?action_type=delete&id=<?php echo $row['Id_Station']; ?>" class="btn btn-danger" onclick="return confirm('هل انت متاكد ؟');">الغاء تفعيل</a>
                    
                    <?php }else{ ?>
                        <a href="UserActionStation.php?action_type=active&id=<?php echo $row['Id_Station']; ?>" class="btn btn-success">تفعيل</a>
                        <?php } ?>
                    </td>
            </tr>
            <?php } }else{ ?>
            <tr><td colspan="7">لا يوجد محطات مضافة</td></tr>
            <?php } ?>
        </tbody>
